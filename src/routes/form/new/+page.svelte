<script lang="ts">
  ...
  function deleteSection(index: number){
    console.log(index);
    data.form.sections = [...data.form.sections.slice(0, index), ...data.form.sections.slice(index + 1)];
  }
...
<form class="mt-1 flex flex-col gap-1 ">
  {#each data.form.sections as section, index}
    <span class="invisible absolute">{ selectedComponent = components.find(comp => comp.type === section.type) }</span>
    <div class="rounded overflow-clip w-full z-10 opacity-80 p-2 flex flex-col gap-y-0.5 bg-background-darker rounded mr-0.5 relative">
      <svelte:component this={selectedComponent.component} editable={false} {section} />
      <!-- Section delete -->
      <button on:click={() => deleteSection(index)} class="flex opacity-80 hover:opacity-100 w-fit absolute right-1 top-1 material-symbols-outlined" type="button">
        delete
      </button>
    </div>
  {/each}
