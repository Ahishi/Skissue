...
<div class="flex h-screen">
    ...
    <nav class="mx-2 h-3 flex relative my-1">
      ...
      <div class="flex rounded-full opacity-80 border overflow-clip mx-1 my-0.25 text-sm">
        <button on:click={() => $viewState = 0}
                class="w-[7rem] border-r bg-secondary-80 bg-opacity-0 hover:bg-opacity-80 transition">All
        </button>
        <button on:click={() => $viewState = 1}
                class="w-[7rem] bg-secondary-80 bg-opacity-0 hover:bg-opacity-80 transition">Group
        </button>
        <button on:click={() => $viewState = 2}
                class="w-[7rem] border-l bg-secondary-80 bg-opacity-0 hover:bg-opacity-80 transition">Personal
        </button>
      </div>
      ...
      <div class="flex rounded-full opacity-80 border overflow-clip my-0.25 text-sm ml-auto">
        <button class="w-[7rem] border-r bg-secondary-80 bg-opacity-0 hover:bg-opacity-80 transition">New</button>
        <button class="w-[7rem] bg-secondary-80 bg-opacity-0 hover:bg-opacity-80 transition">History</button>
      </div>
    </nav>
  ...
  {#if openGroup}
    <div transition:fade={{ duration: 150 }}
         ...
         use:clickOutside={() => { openGroup = false }}>
      ...
      <div class="bg-background-darker relative h-full w-full rounded">
        ...
        <section class="absolute bottom-1 w-full px-1">
          <button class="block bg-error w-full rounded py-1 font-medium opacity-80 transition hover:opacity-100 flex">
  ...
  <aside
    ...
    <button class="block mx-1 rounded font-medium transition flex overflow-clip {newGroupMenu ? '' : 'bg-primary opacity-80'}" on:mouseover={handleMouseOver}>
      {#if newGroupMenu}
        <div class="w-full flex" on:mouseleave={handleMouseOut}>
          <button class="w-1/2 py-1 opacity-80 hover:opacity-100 font-medium transition bg-primary">Join</button>
          <button class="w-1/2 py-1 opacity-80 hover:opacity-100 font-medium transition bg-primary">Create</button>
        </div>
        {:else}
    ...
    <div class="flex block mx-1 my-1 pt-1 mt-auto justify-between bg-background-darker">
      <Avatar />
      <a href="/settings" class="material-symbols-outlined opacity-80 h-2 aspect-square text-center transition hover:opacity-100 leading-[140%]">settings</a>
    </div>
