<script lang="ts">
  import { form } from "../../../routes/preset/stores.js";
...
{#each section.options as option, index}
  <div class="flex">
    ...
    <!-- Checkbox -->
    <input class="bg-background-dark rounded-md disabled:opacity-80 text-primary my-auto" type="checkbox" id="{section.id}-{option.id}" disabled={readonly} bind:checked={$form.sections[section.id].options[index].state}>
    ...
    {#if option.inputField}
      <input type="text" class="py-0.25 ml-0.5 my-auto border-x-0 border-t-0 disabled:opacity-60 transition opacity-80 focus:opacity-100 focus:border-white focus:ring-0 border-b  border-b-background-light px-0.5 bg-transparent"
             bind:value={$form.sections[section.id].options[index].inputText} disabled={readonly}>
    {/if}
